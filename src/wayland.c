...
static void wlr_frame_event_buffer(void *data, struct zwlr_screencopy_frame_v1 *zwlr_screencopy_frame_v1,
    ...
    wlr_frame_data_t *frame_data = (wlr_frame_data_t*)data;
    size_t size = height * stride;
    wl_shm_pool_data_t *smh_pool_data;
...
static void wl_registry_event_global(void *data, struct wl_registry *registry,
    ...
    if (strcmp(interface, wl_output_interface.name) == 0) {
        ...
        wl_state_t *state = (wl_state_t*)data;
        struct wl_output *output = (struct wl_output*)wl_registry_bind(state->registry, name, &wl_output_interface, version);
        wl_output_data_t *output_data;
...
ecode_t wl_state_connect(wl_state_t **r_state, const char *name) {
    wl_state_t *state;
    state = *r_state = calloc(sizeof(wl_state_t), 1);
    state->outputs = darray_init(sizeof(wl_output_data_t*), 1);
    ...
    darray_foreach(state->outputs, i_output, output_data_p) {
        wl_output_data_t *output_data = *(wl_output_data_t**)output_data_p;
...
ecode_t wl_output_create(wl_output_data_t **r_output_data, wl_state_t *state, struct wl_output *output) {
    wl_output_data_t *output_data;
    output_data = *r_output_data = calloc(sizeof(wl_output_data_t), 1);
...
ecode_t wl_shm_pool_create(wl_shm_pool_data_t **r_shm_pool_data, wl_state_t *state, size_t size) {
    wl_shm_pool_data_t *shm_pool_data;
    shm_pool_data = *r_shm_pool_data = calloc(sizeof(wl_shm_pool_data_t), 1);
    int fd;
...
ecode_t wl_buffer_create(wl_buffer_data_t **r_buffer_data, wl_shm_pool_data_t *shm_pool_data,
    ...
    buffer_data = *r_buffer_data = calloc(sizeof(wl_buffer_data_t), 1);
    size_t size = height * stride;
    ...
    buffer_data->buffer =
    wl_shm_pool_create_buffer(shm_pool_data->shm_pool, shm_pool_data->used_size, width, height, stride, format);
...
ecode_t wlr_frame_create(wlr_frame_data_t **r_frame_data, wl_state_t *state,
    ...
    wlr_frame_data_t *frame_data;
    frame_data = *r_frame_data = calloc(sizeof(wlr_frame_data_t), 1);
...
ecode_t wlr_frame_copy(wlr_frame_data_t *frame_data) {
...
ecode_t image_wlr_screencopy(image_t *r_image, wl_state_t *state, rect_t region) {
    darray_t *frames = darray_init(sizeof(wlr_frame_data_t*), 1);
    uint32_t n_requested = 0, n_ready = 0;
    darray_foreach(state->outputs, i_output_data, output_data_p) {
        WS_LOGF(WS_SEV_INFO, "framing output %i\n", i_output_data);
        wl_output_data_t *output_data = *(wl_output_data_t**)output_data_p;
        rect_t inter = rect_inter(output_data->area, region);
        if (!rect_is_valid(inter))
            continue;

        WS_LOGF(WS_SEV_INFO, "output %i framed\n", i_output_data);
        wlr_frame_data_t *frame_data;
    ...
    WS_LOGF(WS_SEV_INFO, "%i outputs framed\n", n_requested);
    ...
    darray_foreach(frames, i_frame_data, frame_data_p) {
        WS_LOGF(WS_SEV_INFO, "imaging frame %i\n", i_frame_data);
        wlr_frame_data_t *frame_data = *(wlr_frame_data_t**)frame_data_p;
        image_t image_part;
