...
static size_t response_write_callback(char *data, size_t data_size, size_t data_count, void *darray) {
    ...
    darray_pop((darray_t*)darray);
    darray_append_array((darray_t*)darray, data, data_size * data_count);
    darray_append((darray_t*)darray, &null_byte);
}

static ecode_t get_headers_slist(curl_slist_t **r_slist, json_object_t headers_json) {
...
static ecode_t set_mime_forms(curl_mime_t *mime, json_object_t forms_json) {
    json_object_object_foreach(forms_json, key_str, val_json) {
        if (!json_object_is_type(val_json, json_type_string))
            continue;

        const char *val_str = json_object_get_string(val_json);
        curl_mimepart_t *mime_part = curl_mime_addpart(mime);
...
static ecode_t append_curl_url_querys(curl_url_t *url, json_object_t querys_json) {
...
static ecode_t get_curl_url(curl_url_t **r_url, json_object_t url_json, json_object_t querys_json) {
    const char *uploader_url_str = json_object_get_string(url_json);
    curl_url_t *url = curl_url();
...
ecode_t request_generic_file(json_object_t *r_response_json, json_object_t uploader_json,
    ...
    json_object_t upload_method_json =
        json_object_object_get(uploader_json, "upload_method");
...
ecode_t request_get(json_object_t *r_response_json, json_object_t uploader_json) {
    ...
    curl_mime_t *mime = curl_mime_init(curl);
    if (!mime)
        return WSE_POST_CURLF;
    ...
    darray_t *response_data = darray_init(1, 16);
...
#endif
    curl_code_t code = curl_easy_perform(curl);
